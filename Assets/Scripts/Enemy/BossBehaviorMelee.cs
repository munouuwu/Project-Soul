...
public class BossBehaviorMelee : MonoBehaviour
{
	public float[] damageGive = new float[2] { 20f, 30f };
	...
	public Vector2 attackRate = new Vector2(0f, 0.5f);
	...
	void Start()
	{
		...
		playerHealth = gameObject.GetComponent<EnemyAI>().target.GetComponent<Health>();
	}
	...
	void Update()
	{
		...
		if (enemyDistance < minAttackDistance)
		{
            //Debug.Log(health.CurrentHP / health.MaxHP);
            if (attackRate.x > 0)
            {
				attackRate.x -= 1f * Time.deltaTime;
            }
            else
            {
				if(health.CurrentHP / health.MaxHP < 0.5f)
                {
					playerHealth.TakeDamage(damageGive[0]);
					animator.SetTrigger("Attack1");
                }
                else
                {
					playerHealth.TakeDamage(damageGive[1]);
					animator.SetTrigger("Attack2");
				}

				attackRate.x = attackRate.y;
